...
<body>
    <div class="game">
        <div class="ui">
            <div>宝石: <span id="gemCount">0</span>/10（通关目标）</div>
    ...
    <script>
        ...
        function movePlayer(targetX, targetY) {
            if(Math.abs(targetX-playerPos.x)+Math.abs(targetY-playerPos.y)!==1) return; // 只能上下左右移动
            // 移除旧玩家位置
        ...
        function handleCellEvent(type, cell) {
            switch(type){
                ...
                case 'npc': showMsg('NPC: 前面有宝箱！🎁'); break;
                case 'trap': showMsg('踩到陷阱！宝石-1😱'); gemCount = Math.max(0,gemCount-1); dom.gemCount.textContent=gemCount; break;
                case 'chest': const reward = Math.floor(Math.random()*3)+1; gemCount+=reward; dom.gemCount.textContent=gemCount; showMsg(`打开宝箱！获得${reward}颗宝石✨`); gemAnimation(cell,reward); cell.classList.remove('chest'); cell.textContent=''; checkWin(); break;
            }
        }
        ...
        // 通关检测
        function checkWin() {
            if(gemCount>=10) showMsg(`恭喜通关！收集了${gemCount}颗宝石🎉`, true);
        }
